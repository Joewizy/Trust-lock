...
contract TrustLockFuzzTest is Test {
    ...
    function testFuzz_ContributionsAndRefunds(
        ...
        // Bound inputs to reasonable ranges (max 2% of 100 ETH goal = 2 ETH per contributor)
        vm.assume(contributionAmount1 >= 0.001 ether && contributionAmount1 <= 2 ether);
        vm.assume(contributionAmount2 >= 0.001 ether && contributionAmount2 <= 2 ether);
        vm.assume(contributionAmount3 >= 0.001 ether && contributionAmount3 <= 2 ether);
    ...
    function testFuzz_MilestoneApprovalsAndRefunds(
        ...
        // Bound to valid milestone percentages (5-25)
        vm.assume(milestone1Percent >= 5 && milestone1Percent <= 25);
        vm.assume(milestone2Percent >= 5 && milestone2Percent <= 25);
        ...
        vm.startPrank(creator);
        trustLock.createMilestone(creator, campaignId, "Milestone 1", milestone1Percent);
        trustLock.createMilestone(creator, campaignId, "Milestone 2", milestone2Percent);
        vm.stopPrank();
